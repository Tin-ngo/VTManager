        ...
        <div class="card">
          ...
          <div class="card-body">
            ...
            <div class="form-group">
              <div class="form-group">
                <label class="col-lg-3 text-nowrap">OWNER ID</label>
                <div class="row align-items-baseline">
                  <div class="col-lg-12">
                    <input v-model="authEmployee.ownerId" type="number" class="form-control border border-dark" autocomplete="off"
                       />
                    <span v-if="errorMessage.owner_id" class="text-danger d-block mt-2">{{ errorMessage?.owner_id }}</span>
                  </div>
                </div>
              </div>
              <div class="form-group">
                <label class="col-lg-3 text-nowrap">EMPLOYEE ID</label>
                <div class="row align-items-baseline">
                  <div class="col-lg-12">
                    <input v-model="authEmployee.employeeId" type="number" class="form-control border border-dark" autocomplete="off"
                       />
                    <span v-if="errorMessage.employee_id" class="text-danger d-block mt-2">{{ errorMessage?.employee_id }}</span>
                  </div>
                </div>
              </div>
              <div class="form-group">
                ...
                <div class="row align-items-baseline">
                  <div class="col-lg-12">
                    <input v-model="authEmployee.password" type="password" class="form-control border border-dark"
                      autocomplete="off"  />
...
export default {
  ...
  methods: {
    ...
    async login() {
      this.isLoading = true;
      await this.loginEmployee()
      this.isLoading = false;
    },
    async loginEmployee(){
      const params = {
        owner_id: this.authEmployee.ownerId,
        employee_id: this.authEmployee.employeeId,
        password: this.authEmployee.password,
        language: this.locale,
      };
      try {
        const response = await authService.loginEmployee(params);
